...
import isInteger, { RANGE_INDEX } from '../validators/isInteger'; // attention: since mobx will wrap the object in a Proxy, the source value will be different from the wrapped one
...
export const addEntry = (obj, key, value, numberAsArrayIndex) => {
  if (numberAsArrayIndex && isInteger(key, {
    range: RANGE_INDEX
  })) {
  ...
  if (length > 0) {
    ...
    while (nested != null && index < lastIndex) {
      ...
      if (next == null || typeof next !== 'object') {
        ...
        if (options.numberAsArrayIndex && isInteger(nextKey, {
          range: RANGE_INDEX
        })) {
